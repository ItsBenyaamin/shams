...
async fn get_today_as_shamsi(body: &Html) -> String {
    ...
    let day_name = full_date.split('-').collect::<Vec<&str>>()[0]
        .trim()
        .to_string();
    let splinted_date = date.split('/').collect::<Vec<&str>>();
    let month = format!(
        "{}({})",
        get_shamsi_month_name(splinted_date[1]),
        splinted_date[1]
    );
...
async fn get_today_as_georgian(body: &Html) -> String {
    ...
    let month = format!(
        "{}({})",
        get_georgian_month_name(splinted_date[1]),
        splinted_date[1]
    );
    let formatted_value = format!(
        "{}: {}  {}  {} {}  {}",
...
async fn get_columns_list(body: &Html) -> Vec<Column> {
    ...
    divs_in_calendar.for_each(|x| {
        let _is_today = x
            .value()
            .classes()
            .filter(|class| class.trim().eq("today"))
            .count()
            > 0;
        let _is_holiday = x
            .children()
            ...
            .unwrap()
            .classes()
            .filter(|class| class.to_string().eq("holiday"))
            .count()
            > 0;
